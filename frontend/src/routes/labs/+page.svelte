...
<div class="flex-1 flex flex-col">
  ...
  <main class="flex-1 p-6 overflow-auto space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h2 class="text-xl font-semibold">Labs</h2>
        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.2em] text-gray-500">
          <button class="hover:text-white" on:click={goToRoot}>Root</button>
          {#if currentFolder}
            <span>/</span>
      ...
      <div class="flex flex-wrap items-center gap-2">
        <button
          class="rounded-md border border-gray-700 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-gray-300 hover:border-blue-500 hover:text-white disabled:opacity-40"
        ...
        <button
          class="rounded-md border border-blue-500 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-blue-200 hover:bg-blue-500/10 disabled:opacity-40"
          disabled={busy}
          on:click={createLab}
        >
          New Lab
    ...
    {:else if error}
      <div class="max-w-xl rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-100">
        ...
        <button class="mt-4 rounded-md border border-red-400/40 px-3 py-2 text-xs uppercase tracking-[0.2em] text-red-100 hover:bg-red-500/10" on:click={() => fetchContents(currentFolder)}>
          Retry
        </button>
      </div>
    {:else}
      <section class="space-y-3">
        <div class="text-xs uppercase tracking-[0.2em] text-gray-500">Folders</div>
        {#if folders.length === 0}
          <div class="rounded-lg border border-gray-800 bg-gray-900 p-4 text-sm text-gray-400">No folders in {currentFolderLabel || '/'}.</div>
        {:else}
            ...
            {#each folders as folder}
              ...
              <article class="rounded-lg border border-gray-700 bg-gray-800 p-4">
                <button class="w-full text-left" on:click={() => openFolder(folder)}>
                  ...
                  <div class="mt-2 text-xs uppercase tracking-[0.2em] text-gray-500">{folder.path}</div>
                  {#if folder.mtime}
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                  <button
                    class="rounded-md border border-gray-700 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-gray-300 hover:border-blue-500 hover:text-white"
                  ...
                  <button
                    class="rounded-md border border-red-500/40 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-red-200 hover:bg-red-500/10"
                    disabled={isStaticFolder}
      ...
      <section class="space-y-3">
        <div class="text-xs uppercase tracking-[0.2em] text-gray-500">Labs</div>
        {#if labs.length === 0}
          <div class="rounded-lg border border-gray-800 bg-gray-900 p-4 text-sm text-gray-400">No labs in {currentFolderLabel || '/'}.</div>
        {:else}
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            {#each labs as lab}
              <article class="rounded-lg border border-gray-700 bg-gray-800 p-4">
